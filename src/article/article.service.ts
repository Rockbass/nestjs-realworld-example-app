...
export class ArticleService {
  ...
  async favorite(id: number, slug: string): Promise<ArticleRO> {
    ...
    const isNewFavorite =
      user.favorites.findIndex(art => art.id === article.id) < 0;
    if (isNewFavorite) {
  ...
  async unFavorite(id: number, slug: string): Promise<ArticleRO> {
    ...
    const deleteIndex = user.favorites.findIndex(art => art.id === article.id);
